...
import { launchesByYear, launchesByYearStacked, launchesByMonth, launchesByMonthStacked, MONTH_LABELS } from '../data/aggregator.js';
...
export function createLaunchesOverTime(container) {
  ...
  backBtn.addEventListener('click', () => {
    drillYear = null;
    backBtn.style.display = 'none';
    heading.textContent = 'Launches Over Time';
    if (prevYearRange) {
      filterState.set(prevYearRange);
      prevYearRange = null;
    }
  });
  ...
  function drillInto(year) {
    ...
    prevYearRange = { yearMin: state.yearMin, yearMax: state.yearMax };
    drillYear = year;
    backBtn.style.display = '';
    heading.textContent = `Launches in ${year}`;
    filterState.set({ yearMin: year, yearMax: year });
  }
  ...
  function draw() {
    ...
    const isStacked = chartMode === 'stacked' || chartMode === 'stackedAgency' || chartMode === 'stackedVehicle';

    if (drillYear !== null) {
      if (isStacked) {
        drawStackedMonthly(g, launches, innerW, innerH);
      } else {
        drawAreaMonthly(g, launches, innerW, innerH);
      }
    } else if (isStacked) {
      drawStacked(g, launches, innerW, innerH);
    } else {
      drawArea(g, launches, payloads, innerW, innerH);
    }
  }

  function drawArea(g, launches, payloads, innerW, innerH) {
  ...
  function drawAreaMonthly(g, launches, innerW, innerH) {
    const data = launchesByMonth(launches, drillYear);
    ...
    g.selectAll('.bar')
      ...
      .on('mousemove', (event, d) => {
        tooltip.show(
          `<div class="tt-title">${MONTH_LABELS[d.month]} ${drillYear}</div>
           <div class="tt-row"><span class="tt-label">Launches</span><span class="tt-value">${fmtNum(d.count)}</span></div>`,
          event
        );
      })
  ...
  function drawStacked(g, launches, innerW, innerH) {
  ...
  function drawStackedMonthly(g, launches, innerW, innerH) {
    const { data, keys } = launchesByMonthStacked(launches, drillYear, stackField);
    ...
    g.selectAll('.hover-bar')
      ...
      .on('mousemove', (event, d) => {
        ...
        tooltip.show(`<div class="tt-title">${MONTH_LABELS[d.month]} ${drillYear}</div>${rows}`, event);
      })
      .on('mouseleave', () => tooltip.hide());
  ...
  return {
    update(launches, payloads, filters) {
      currentData = { launches, payloads };
      // Exit drill mode if year range no longer matches the drilled year
      if (drillYear !== null && filters) {
        if (filters.yearMin !== drillYear || filters.yearMax !== drillYear) {
          drillYear = null;
          prevYearRange = null;
          backBtn.style.display = 'none';
          heading.textContent = 'Launches Over Time';
